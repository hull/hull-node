...
module.exports = {
  ...
  signUserData: function (userData, secret) {
    ...
    var signature = crypto.createHmac('sha1', secret || configuration.get().appSecret).update([message, timestamp].join(' ')).digest('hex');
    var parts = [
      message,
      signature,
      timestamp
  ...
  checkSignedUserId: function(userId, userSig, secret) {
    ...
    var sig       = userSig.split("."),
        time      = sig[0],
        signature = sig[1],
        digest    = crypto.createHmac('sha1', secret || conf.get().appSecret).update([time, userId].join("-")).digest('hex');
