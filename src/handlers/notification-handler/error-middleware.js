  ...
  return function notificationHandlerErrorMiddleware(err: Error, req: HullRequestFull, res: $Response, next: NextFunction) {
    ...
    if (typeof req.hull.notification !== "object") {
      ...
      if (err instanceof NotificationValidationError) {
        const flowControl = notificationDefaultFlowControl(req.hull, "ship:update", "error");
        res.status(400).json({ flow_control: flowControl, error: errorToResponse(err) });
      } else {
        res.status(400).json({ error: "didn't get a correct notification", message: err.message });
      }
      return next(err);
    }
    ...
    if (err.message === "Channel unsupported") {
      ...
      return res.status(200).json({ flow_control: defaultUnsupportedFlowControl, error: errorToResponse(err) });
    }
    ...
    if (err instanceof TransientError) {
      return res.status(503).json({ flow_control: defaultErrorFlowControl, error: errorToResponse(err) });
    }

    res.status(500).json({ flow_control: defaultErrorFlowControl, error: errorToResponse(err) });
    return next(err);
  };
}
