...
describe("HullConnector", () => {
  ...
  it("should allow passing name to clientConfig and to Hull Middleware", () => {
    ...
    const connector = new HullConnector(HullStub, middlewareSpy, { connectorName: "example" });
    expect(connector.clientConfig.connectorName).to.be.eql("example");
    connector.clientMiddleware();
    expect(middlewareSpy.getCall(0).args[0].clientConfig).to.be.eql({ connectorName: "example" });
  });
  ...
  it("should allow to set the name of internal queue", () => {
    ...
    const connector = new HullConnector(HullStub, HullMiddlewareStub, { queue });
    connector.worker();
